...
app = Flask(__name__)
app.config["SECRET_KEY"] = "th!$-is-+he-KEY"
...
def index():
    ...
    f = open("docs.txt", "w")
    f.write(str(docs_list) + "\n\nP.S. Passwords are not stored here... if that's what you're looking for...")
    f.close()

    return render_template("index.html", total=total, docs_list=docs_list, preview=preview)


@app.route("/find", methods=['GET'])
def search():
    if request.args.get('term').find("\'") != -1 or request.args.get('term').find("%") != -1:
...
def createDoc():
    ...
    if password1 == password2:
        ...
        author = f" {author}"
        if title.isspace() or title.find("\'") != -1 or title.find("%") != -1:
            error = "Invalid title name! Title cannot be just be spaces, contain any single quotes ('), or contain any percent signs (%)."
            return render_template("system/error-report.html", error=error)
...
def viewDoc(title):
    ...
    doc = doc[3:]
    doc = doc[:-4]
    return render_template("open/index.html", title=title, content=doc)
